...
- 国内论坛板块内容轮换：`generate_cn_forums.py` 在抓取阶段读取 `~/.ai-signal/seen.json`，已推送过的话题直接跳过、名次顺延给下一位候选；板块新鲜量不足 5 条时启用递补池（同主题/时间窗门槛，仅放宽互动数）补齐；各站热榜同样剔除已推送 id 轮换。雪球搜索候选池从每词 10 条扩到 20 条（page1+2）。
  之所以这样改：搜索型/热度型板块（尤其雪球）更新慢，同一批热帖连续多日霸榜，被 prepare_digest 的 seen 过滤后板块整块消失——用户看到"为什么今天没有雪球"。轮换把"重复即消失"改成"重复即顺延"，保证每个板块每天都给日报留足没见过的新内容，同时递补池有主题词和时间窗兜底，不会为凑数放进低质量噪音。
  轮换核心逻辑抽为 `rotate_fresh()` 纯函数，配 `tests/test_cn_forums_rotation.py` 七个用例（跳过顺延/递补保底/上限截断/空态安全等）。
