...
def load_seen():
    ...
    if SEEN_PATH.exists():
        try:
            ...
            seen = {}
    for key in ("tweets", "episodes", "papers", "articles", "cn_forums"):
        seen.setdefault(key, {})
    return seen
...
def filter_unseen(feed_x, feed_podcasts, papers, articles, seen, cn_forums=None):
    now = datetime.now(timezone.utc).isoformat()
    new_ids = {"tweets": [], "episodes": [], "papers": [], "articles": [], "cn_forums": []}
    emitted = {"tweets": set(), "episodes": set(), "papers": set(), "articles": set(), "cn_forums": set()}
    ...
    fresh_cn_forums = []
    for topic in cn_forums or []:
        fid = str(topic.get("id") or topic.get("url") or "")
        if fid and (fid in seen.get("cn_forums", {}) or fid in emitted["cn_forums"]):
            continue
        if fid:
            emitted["cn_forums"].add(fid)
            new_ids["cn_forums"].append(fid)
        fresh_cn_forums.append(topic)

    marks = {kind: {i: now for i in ids} for kind, ids in new_ids.items()}
    return accounts, episodes, fresh_papers, fresh_articles, marks, fresh_cn_forums
...
def main():
    ...
    seen = load_seen()
    # Local CN forums feed (subscriber-side; generated by generate_cn_forums.py)
    feed_cn_forums = load_local_json("feed-cn-forums.json")
    cn_forums_topics = (feed_cn_forums or {}).get("topics", [])
    # 热榜：每站独立分组（hot_boards），不做跨站合并
    cn_forums_hot = []
    for board in (feed_cn_forums or {}).get("hot_boards", []) or []:
        for it in board.get("items", []):
            entry = dict(it)
            entry["hot_board_source"] = board.get("source_name") or board.get("source")
            cn_forums_hot.append(entry)
    if args.include_seen:
        ...
        cn_forums = cn_forums_topics + [t for t in cn_forums_hot if t.get("hot")]
        marks = {"tweets": {}, "episodes": {}, "papers": {}, "articles": {}, "cn_forums": {}}
    else:
        x_accounts, episodes, papers, articles, marks, cn_forums = filter_unseen(
            feed_x,
            feed_podcasts,
            (feed_arxiv or {}).get("papers", []),
            (feed_blogs or {}).get("articles", []),
            seen,
            cn_forums_topics + cn_forums_hot,
        )
    ...
    stats = {
        ...
        "cn_forum_topics": len(cn_forums),
        "cn_forum_hot": sum(1 for t in cn_forums if t.get("hot")),
    }
    config_out = {
        "language": language,
    ...
    manifest = {
        ...
        "cn_forums": [
            {
                "source": t.get("source_name") or t.get("source"),
                "board": t.get("board"),
                "title": t.get("title"),
                ...
                "theme": t.get("theme"),
                "hot": t.get("hot", False),
                "hot_rank": t.get("hot_rank"),
                "url": t.get("url"),
            }
            # 排序：热榜在前（按站分组、站内按 hot_rank），主题帖在后
            for t in sorted(
                cn_forums,
                key=lambda x: (
                    0 if x.get("hot") else 1,
                    x.get("source", ""),
                    x.get("hot_rank") or 99,
                ),
            )
        ],
